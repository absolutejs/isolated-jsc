...
- **[oven-sh/bun#5602](https://github.com/oven-sh/bun/issues/5602)** — "Fix `expect().rejects.toThrow()`" — **open** since 2023-09-17, labelled `bug`, `bun:test`. The tracking umbrella.
- **[oven-sh/bun#14670](https://github.com/oven-sh/bun/issues/14670)** — "Timeout in test when expecting promise from bun shell to throw" — **open** since 2024-10-18, labelled `bug`, `bun:test`, `shell`, **`confirmed bug`**. Same symptom against `$\`bad-command\``.
- **[oven-sh/bun#19130](https://github.com/oven-sh/bun/issues/19130)** — "`bun test` hangs up in timeout with PostgreSQL and `expect->toThrow()`" — **open** since 2025-04-19. Exact same hang pattern against a PostgreSQL handle.
...
We have not filed a Worker-flavoured variant; the three open issues
already cover the underlying matcher bug, and adding a fourth flavour
wouldn't add signal. Once #5602 closes we'll verify against our Worker
repro before deleting the workaround.
...
1. Confirm against this repro:

   ```ts
   import { expect, test } from "bun:test";
   import { createIsolate } from "@absolutejs/isolated-jsc";

   test("rejects.toThrow works again", async () => {
     const isolate = await createIsolate();
     const context = await isolate.createContext();
     const script = await isolate.compileScript('throw new Error("boom")');
     await expect(script.run(context)).rejects.toThrow("boom"); // should resolve in ~1 ms
...
5. Comment on whichever of #5602 / #14670 / #19130 / #23420 was the one
   that closed, linking the deleted workaround.
